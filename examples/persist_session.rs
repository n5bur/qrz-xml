...
use qrz_xml::{QrzXmlClient, ApiVersion, QrzXmlError};
...
impl CachedSession {
    fn new(session_key: String, username: String, expires_at: Option<String>, lookup_count: Option<u32>) -> Self {
...
impl PersistentQrzXmlClient {
    ...
    async fn ensure_authenticated(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if let Some(cached) = self.store.load_session(&self.username) {
            println!("Found cached session from {}", 
            ...
            match self.client.lookup_callsign("AA7BQ").await {
                ...
                Err(QrzXmlError::SessionExpired | QrzXmlError::NoSessionKey | QrzXmlError::ApiError { .. }) => {
    ...
    async fn lookup_callsign(&mut self, callsign: &str) -> Result<qrz_xml::CallsignInfo, Box<dyn std::error::Error>> {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut client = PersistentQrzXmlClient::new(
        &username,
        &password,
        ApiVersion::Current,
    )?;
    ...
    println!("Found: {} - {}", info.call, info.full_name().unwrap_or_default());
